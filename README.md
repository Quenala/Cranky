...
Download / copy the Cranky folder into your Windower addons directory:
...
(Optional) Add it to your scripts/init.txt so it loads automatically:
...
Show / hide weapon skill names





//cranky toggle damage
...
Set column order (any combination of name, ws, damage)
...
Toggle the shake effect on 99999 damage
...
Set how many WS lines to display (1–20)
...
Job abbreviations are taken from party/alliance data and update packets. Your own job appears immediately; other players appear once the game sends their job information.



The GUI uses a monospace font (Consolas) so columns stay aligned.



Only the most recent weapon skills are kept (oldest ones are removed when the row limit is reached).
